...
vector<string> findWikiLadder(const string& start_page, const string& end_page) {
    ...
    WikiScraper scraper;
    unordered_set<string> visitedPage;
    auto target_set = scraper.getLinkSet(end_page);
    auto cmpFn = [&](vector<string> ladder1, vector<string> ladder2) {
        auto page1_link_set = scraper.getLinkSet(ladder1.back());
        auto page2_link_set = scraper.getLinkSet(ladder2.back());
        int num1 = 0, num2 = 0;
        for (string link : target_set) {
            if (page1_link_set.find(link) != page1_link_set.end()) {
                num1++;
            }
            if (page2_link_set.find(link) != page2_link_set.end()) {
                num2++;
            }
        }
        return num1 < num2;
    ...
    ladderQueue.push(start_ladder);
    while (!ladderQueue.empty()) {
        vector<string> topLadder = ladderQueue.top();
        auto currPageLinks = scraper.getLinkSet(topLadder.back());
        ladderQueue.pop();
        if (currPageLinks.find(end_page) != currPageLinks.end()) {
            topLadder.push_back(end_page);
            return topLadder;
        }

        for (auto& neighborPage : currPageLinks) {
            if (visitedPage.find(neighborPage) == visitedPage.end()) {
                vector<string> copy(topLadder);
                copy.push_back(neighborPage);
                ladderQueue.push(copy);
                visitedPage.insert(neighborPage);
            }
        }
    }
    return {};
