...
M& HashMap<K, M, H>::operator[](const K& key) {
    ...
    return contains(key) ? const_cast<HashMap<K, M, H>*>(this)->at(key) : insert({key, {}}).first->second;
...
bool operator==(const HashMap<K, M, H>& lhs, const HashMap<K, M, H>& rhs) {
    ...
    //     if (rhs.find(iter->first) == rhs.end() || rhs.find(iter->first)->second != iter->second) return false;
    // }
    // if (lhs.size() != rhs.size()) return false;
    // return true;
    // END STARTER CODE
...
// copy constructor
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(const HashMap& other) : HashMap(other.begin(), other.end(), other.bucket_count(), other._hash_function) {
