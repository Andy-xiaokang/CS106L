...
MyVector<T>::MyVector(size_type n, const_reference val) :
    _logicalSize(n),
    _allocatedSize(std::max(kDefaultSize, 2*_logicalSize)),
    _elems(new T[_allocatedSize])
{
...
template <typename T>
MyVector<T>::MyVector(std::initializer_list<value_type> init) :
...
MyVector<T>& MyVector<T>::operator=(const MyVector<T>& rhs) {
    if (this != &rhs) {
        ...
        delete [] _elems;
        // copy over all variables, deep copying where necessary
