...
pair<InputIt1, InputIt2> my_mismatch(InputIt1 first1, InputIt1 last1, InputIt2 first2) {
    ...
    if (first1 == last1) return {last1, first2};
...
int main() {
    ...
    auto [first3, second3] = my_mismatch(testDeq1.begin(), testDeq1.end(), testDeq2.begin());
    cout << *first3 << " " << *second3 << endl;

    auto [first4, second4] = my_mismatch(testDeq1.begin(), testDeq1.end(), testDeq1.begin());
    cout << *first4 << "" << *second4 << endl;
}
